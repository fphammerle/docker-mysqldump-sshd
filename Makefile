...
IMAGE_NAME = docker.io/fphammerle/mysqldump-sshd
PROJECT_VERSION = $(shell git describe --match=v* --abbrev=0 --dirty | sed -e 's/^v//')
MARIADB_CLIENT_PACKAGE_VERSION = $(shell grep -Po 'MARIADB_CLIENT_PACKAGE_VERSION=\K.+' Dockerfile | tr -d -)
...
IMAGE_TAG = ${PROJECT_VERSION}-mariadb${MARIADB_CLIENT_PACKAGE_VERSION}-openssh${OPENSSH_SERVER_PACKAGE_VERSION}-${IMAGE_TAG_ARCH}
